...
train_X = [imageio.imread("p2_data/"+n) for n in train_set_name]
train_X = np.array(train_X).reshape(240,-1)
train_y = [i for i in range(1,41) for _ in range(1,7)]

test_X = [imageio.imread("p2_data/"+n) for n in test_set_name]
...
input_img = imageio.imread("p2_data/1_1.png").reshape(1,-1)
