...
optimizer = optim.SGD(list(encoder.parameters()) + list(classifier.parameters()),  lr=learningRate, weight_decay = weightDecay,momentum = 0.9)
...
for currentEpoch in tqdm(range(numEpochs)):
    for bid, (data, labels) in enumerate(tqdm(train_loader)):
        ...
        start_steps = currentEpoch * len (train_loader)
        total_steps = numEpochs * len(train_loader)
        p = float(currentEpoch + start_steps) / total_steps
...
torch.save(encoder, './MNIST2MNISTmodel')
torch.save(classifier, './MNIST2MNISTclassifier')
