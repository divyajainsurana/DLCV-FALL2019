...
sourceTestDataset =  MNIST(csv_file="./hw3_data/digits/svhn/test.csv", root_dir="./hw3_data/digits/svhn/test",transform=transforms.Compose([
#                                transforms.Resize(image_size),
#                                transforms.CenterCrop(image_size),
                               transforms.ToTensor(),
                               transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
...
if ~tcuda.is_available():
    sourceCNN = torch.load('SVHN2SVHNmodel',map_location=lambda storage, loc: storage)
else:
    sourceCNN = torch.load('SVHN2SVHNmodel')

sourceCNN.eval()

targetCNN = Extractor()
if ~tcuda.is_available():
    targetCNN = torch.load('SVHN2SVHNmodel',map_location=lambda storage, loc: storage)
else:
    targetCNN = torch.load('SVHN2SVHNmodel')

targetCNN.train()

classifier = Classifier()
if ~tcuda.is_available():
    classifier = torch.load('SVHN2SVHNclassifier',map_location=lambda storage, loc: storage)
else:
    classifier = torch.load('SVHN2SVHNclassifier')
...
torch.save(targetCNN, 'targetTrainedModelSVHNtoMNIST_improved')
